...
ax_a.set_xlabel("Δ (bias_multi − bias_unique) [log₂]", fontsize=12)
ax_a.set_ylabel("Number of gene-dataset pairs", fontsize=12)
ax_a.set_title("A   Genome-wide Δ distribution", fontsize=13, fontweight="bold", loc="left")
...
ax_b.set_xlabel("bias_unique (primary pipeline) [log₂]", fontsize=12)
ax_b.set_ylabel("bias_multimap (−M −−fraction) [log₂]", fontsize=12)
ax_b.set_title("B   Catalog genes: unique vs multi bias", fontsize=13, fontweight="bold", loc="left")
...
ax_b.legend(handles=handles, fontsize=10, loc="upper left")
...
ax_c.set_yticklabels([f"{'*' if r['mclass']=='SNP-induced multi-mapping' else ''}{r['gene_name']}"
                       for _, r in per_gene.iterrows()], fontsize=9)
ax_c.set_xlabel("Residual fraction (bias_multi / bias_unique)", fontsize=12)
ax_c.set_title("C   Residual fraction per catalog gene", fontsize=13, fontweight="bold", loc="left")
ax_c.set_xlim(-0.55, 1.75)
ax_c.text(1.02, 0.01, "← multi-mapping\n   dominant", transform=ax_c.transAxes,
          fontsize=9, va="bottom", color=C_MULTI)
...
for gname, gx in group_x.items():
    ...
    col = C_MULTI if "multi-mapping" in mclass else C_DIRECT
    ax_d.text(gx, -0.8, gname, ha="center", va="top", fontsize=10, color=col, fontweight="bold")
    ax_d.text(gx, -1.2, rep_ids[gname][1], ha="center", va="top", fontsize=8, color=col)

ax_d.axhline(0, color="black", lw=0.8)
ax_d.set_xticks([])
ax_d.set_ylabel("Bias [log₂FC_fvb − log₂FC_naive]", fontsize=12)
ax_d.set_title("D   Representative genes: unique vs multi bias", fontsize=13, fontweight="bold", loc="left")
...
ax_d.legend(handles=handles_d, fontsize=10, loc="upper right")
